...
主成果物は、**実寸スケールのローカル再構成GLB**。点群、カメラ、品質レポートは再現性とモデリングを支える補助成果物とする。ブラウザ収録はRGBとDepthが未校正のため、まず正確な形状を正本とし、写真テクスチャはネイティブRGB-D収録の拡張経路とする。3DGS は今回の必須経路に含めない。
...
- WebXR/ARCore の深度センサー姿勢・投影は初期値として使い、PC 側で深度座標を補正し、保守的な近傍ICPとTSDF融合を行う。
...
- ARCore/WebXR姿勢を初期値として、近傍フレーム間の保守的なICP補正とOpen3D Scalable TSDF融合で連続した面を生成する。外部の画像 BA/COLMAP は入力経路を置き換えられる拡張点として残し、標準処理は外部ツールなし・CPUだけで完了する。
...
- RGBとDepthが校正済みの収録では色を融合する。WebXRブラウザ収録では未校正 `getUserMedia` RGBを誤って貼らず、形状のみを出力する。
...
- **ARCore/WebXR の姿勢ドリフト**：標準経路では近傍ICPを小さな補正に限定して使い、採用フレーム数をレポートする。画像 BA/COLMAP を使う場合は外部姿勢入力へ差し替えられる構成にする。
...
- [x] Windows 側の品質スコア、キーフレーム選別、WebXR深度座標補正、Open3D TSDF融合、近傍ICP、床面整列、スケールアンカー
- [x] TSDF Raw/軽量化Clean GLB、PLY、カメラ、寸法、HTMLレポート（WebXRは未校正RGBをテクスチャ化しない）
